...
chmod -R 777 /app/var
echo "Installing PHP dependencies..." && composer install --no-interaction --prefer-dist || true
if [ -f /app/package.json ]; then echo "Installing JS dependencies (pnpm)..." && (cd /app && pnpm install --frozen-lockfile || pnpm install) && echo "Building assets..." && (cd /app && pnpm run build) || true; fi
